...
import { useListAllBlogs } from "../../../api/administrator/blogs/getAllBlogs";
import Pagination from "../../../components/common/Pagination";
import { useDeleteBlog } from "../../../api/administrator/blogs/deleteBlog";
...
const ListBlogs = () => {
  ...
  const { data, isLoading, isError, refetch } = useListAllBlogs(
    currentPage,
    limit,
    search,
  );
  ...
  const { mutate: deleteItem, isPending } = useDeleteBlog();
  ...
  if (isError) return <div className="p-6">Error loading blogs.</div>;

  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-800">Blog</h2>
        {hasModule("blog.create") && (
          <button
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                {(hasModule("blog.view") || hasModule("blog.update") || hasModule("blog.delete")) && (
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-3">
                        {hasModule("blog.view") && (
                          <button
                            ...
                            onClick={() => navigate(`/admin/blogs/view`, { state: { item } })}
                          >
                            <Eye size={18} />
                          </button>
                        )}
                        {hasModule("blog.update") && (
                          <button
                            className="text-blue-600 hover:text-blue-900"
                            onClick={() => navigate(`/admin/blogs/edit`, { state: { item } })}
                          >
                            <Pencil size={18} />
                          </button>
                        )}
                        {hasModule("blog.delete") && (
                          <button
